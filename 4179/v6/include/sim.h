 ...
 * dangling bonds to a now-missing atom, which this function refuses
 * to do rather than silently producing an invalid topology).
 *
 * This is real chemistry infrastructure, not a one-off hack: forming
 * a covalent bond via condensation (e.g. a glycosidic bond, an ester,
 * a peptide bond) always involves removing a leaving-group atom from
 * at least one reactant first.
 */
