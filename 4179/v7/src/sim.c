...
int sim_remove_terminal_atom(Simulation *sim, int atom_idx) {
    ...
    for (int b = 0; b < sim->num_bonds; b++) {
        if (sim->bonds[b].atom_a == atom_idx || sim->bonds[b].atom_b == atom_idx) {
            bond_to_remove = b;
            break;
        }
    }
    if (bond_to_remove >= 0) {
        for (int b = bond_to_remove; b < sim->num_bonds - 1; b++)
            sim->bonds[b] = sim->bonds[b+1];
        sim->num_bonds--;
    }
    ...
    for (int b = 0; b < sim->num_bonds; b++) {
        ...
        if (sim->bonds[b].atom_b > atom_idx) sim->bonds[b].atom_b--;
    }
    for (int a = 0; a < sim->num_angles; a++) {
        if (sim->angles[a].atom_a > atom_idx) sim->angles[a].atom_a--;
    ...
    for (int i = atom_idx; i < sim->num_atoms - 1; i++)
        sim->atoms[i] = sim->atoms[i+1];
    sim->num_atoms--;

    /* 5. Re-index every atom's bond_partners[] list: drop the entry
     *    pointing at atom_idx (only the neighbor has one), and
     *    decrement every partner index > atom_idx */
    for (int i = 0; i < sim->num_atoms; i++) {
        Atom *a = &sim->atoms[i];
        int w2 = 0;
        for (int p = 0; p < a->num_bonds; p++) {
            if (a->bond_partners[p] == atom_idx) continue; /* drop */
            int partner = a->bond_partners[p];
            int order   = a->bond_orders[p];
            if (partner > atom_idx) partner--;
            a->bond_partners[w2] = partner;
            a->bond_orders[w2]   = order;
            w2++;
        }
        a->num_bonds = w2;
